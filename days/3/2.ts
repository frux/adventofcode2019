/**
 * Find the intersections of wires with the shortest path
...
function getClosestIntersectionDistance(path1: TPath, path2: TPath, from: TCoordinate): number {
    ...
    for (let path2Dir of path2) {
        ...
        // Move a pointer to the next coordinate and calculate path length
        for (let i = 0; i < length; i++) {
            let path1Length = 0;
            path2Length++;
            ...
            for (let path1Coord of path1Coords) {
                ...
                if (isIntersection(path1Coord, pointer2)) {
                    // Calculate a total length of both pathes at this point
                    let distance = path2Length + path1Length;
